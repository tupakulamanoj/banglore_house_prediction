...
mm=['Electronic City Phase II',
        ...
        'Whietfield']
mm.sort()
def price(location,size,total_sqft):
    data1=pd.read_csv('blg.csv')
    data1.drop('Unnamed: 0',axis=1,inplace=True)
    warnings.filterwarnings("ignore")
    pickled_model = pickle.load(open('model.pkl', 'rb'))
    a=np.zeros(len(data1.columns))
    a[1]=size
    a[0]=total_sqft
    d=data1.columns.get_loc(location)
    a[d]=1
    final_price=pickled_model.predict([a])[0]
    return final_price
...
def blg():
    if request.method == 'POST':
        ...
        a=price(loca,size,sqft)
        price_final=round(a,2)*10
        
        
        return redirect(url_for("banglore", price=price_final,location=loca,distance=sqft))
        
    return render_template('base.html',q=mm)
...
def banglore():
    ...
    return render_template('index2.html',price=price,location=location,distance=distance)
